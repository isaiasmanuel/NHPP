...
def NormalZ(i):
    ...
    for lindex in range(L):
        Zprob[lindex]=Cadena[-1][1][int(SimPoisson[i,3])][lindex]*multivariate_normal.pdf(SimPoisson[i,[0,1]], mean=Cadena[-1][4][lindex][0],cov=Cadena[-1][4][lindex][1])    
    dens=Zprob[:]/np.sum(Zprob[:])
    return np.random.choice(L,p=dens)



def AsignarZ_Todos(SimPoisson, ultimo_estado, L):
    N = SimPoisson.shape[0]
    pesos = ultimo_estado[1]       # Pesos de Beta
    Phi = ultimo_estado[4]         # Medias y Covarianzas
    indices_t = SimPoisson[:, 3].astype(int)
    posiciones = SimPoisson[:, [0, 1]]

    # Matriz para almacenar densidades (N puntos x L componentes)
    probs = np.zeros((N, L))

    for l in range(L):
        # Media y covarianza del componente l
        mu = Phi[l][0]
        cov = Phi[l][1]
        densidad = multivariate_normal.pdf(posiciones, mean=mu, cov=cov)
        probs[:, l] = pesos[indices_t, l] * densidad


    sumas = probs.sum(axis=1)

    probs /= sumas[:, np.newaxis]
    return np.array([np.random.choice(L, p=p) for p in probs])
...
def Post(X):
    ...
    p3=mp.fsum((Alpha[:-1]-1)*np.log(Alpha[1:])-Alpha[1:]-np.array([mp.log(x) for x in GammaVectorized(Alpha[:-1])]))
    p4=0
    for i in range(pT-1):
        prod=Alpha[i+1]*Beta[i]
        if all(prod>0):
            p4+=mp.fsum((prod-1)*np.log(Beta[i+1])) + mp.log(BetaInv(prod))
        else :
...
def PostParalel(m):
    ...
    # with open("SeguimientoP.csv", mode='a', newline='') as f:
    #     writer = csv.writer(f)
    #     writer.writerow([float(p1), float(p2), float(p3), float(p4), float(p5), float(p6), float(p7), float(p8)])
    return p1+p2+p3+p4+p5+p6+p7+p8
...
# # leer csv
# seguimientoP = pd.read_csv("seguimientoP.csv")

# # ver estructura
# print(seguimientoP.head())

# # graficar una serie por cada columna
# for col in seguimientoP.columns:
#     plt.figure()
#     plt.plot(seguimientoP[col])
...
def CadenaMarginalBetaLabel(i,maps,Cadena,MuestraEfectiva):
        # plt.plot(Cadena[IATElegido::IATElegido,i])
        graf=np.zeros(len(MuestraEfectiva))
        contador=0
        for j in MuestraEfectiva:
            graf[contador]=Cadena[j][1][0][maps[contador][i]]
            contador+=1
        plt.plot(graf)
        plt.title(str(i))
        plt.show()          
...
def a(alpha,Beta,i):
    if i==0:
        ...
        num=mp.exp(-alpha[i])*mp.power(alpha[i],(alpha[i-1]-1))*mp.power(alpha[i+1],(alpha[i])) * mp.fprod(Beta[i,:]**(alpha[i]*Beta[i-1,:] ))* BetaInv( mp.mpmathify(alpha[i])*(Beta[i-1,:])) 
        den=mp.gamma(alpha[i])
    else :
        num=mp.exp(-alpha[i])*mp.power(alpha[i],(alpha[i-1]-1)) * mp.fprod(Beta[i,:]**(alpha[i]*Beta[i-1,:] ))* BetaInv( mp.mpmathify(alpha[i])*(Beta[i-1,:])) 
...
def bp(alpha,Beta,Z,p):
    m1l=np.zeros(L)
    for i in range(L):
        m1l[i]=np.sum(Z[SimPoisson[:,3]==p]==i)
        # m1l[i]=np.sum(Z==i)
    if p==0:
        # return mp.fprod(Beta[p,:]**(m1l+alpha[p]/L-1))*mp.fprod(Beta[p+1,:]**(alpha[p+1]*Beta[p,:]))*BetaInv(alpha[p+1]*Beta[p,:])
        return mp.fprod(Beta[p,:]**(m1l+alpha[p]/L-1))*mp.fprod(Beta[p+1,:]**(alpha[p+1]*Beta[p,:]))*BetaInv(alpha[p+1]*mp.matrix((Beta[p,:])))
    else :
        # return mp.fprod(Beta[p,:]**(m1l+alpha[p]*Beta[p-1,:]-1))*mp.fprod(Beta[p+1,:]**(alpha[p+1]*Beta[p,:]))*BetaInv(alpha[p+1]*Beta[p,:])
        return mp.fprod(Beta[p,:]**(m1l+alpha[p]*Beta[p-1,:]-1))*mp.fprod(Beta[p+1,:]**(alpha[p+1]*Beta[p,:]))*BetaInv(alpha[p+1]*mp.matrix((Beta[p,:])))
...
def SALT(beta):
    ...
    aislado=np.copy(coord)
    while aislado==coord   :
        aislado=np.random.choice(len(beta))
    bipropuesto=np.copy(beta)
    bi=beta[coord]
    logitbi=mp.log(mp.fdiv(bi,mp.fsum((1,-bi))))+np.random.normal(size=1,scale=1 )[0]
    ...
    for i in range(len(bipropuesto)):
        if i!=coord and i!=aislado:
            bipropuesto[i]= (1-bipropuesto[coord]) * mp.fsum((( beta[i]/(1-beta[coord]),np.random.uniform(-10**-10,10**-10))))
...
def PostMalla(PDF,CV,opcion,mapa=0):
    ...
    for idx, t in enumerate(CutPoint[:-1]):
        ...
        if opcion==2:
            ...
            contour = ax.contourf(Xg, Yg, pdf0, cmap='viridis',levels=np.linspace(0, vmax, 10000), vmax=vmax)
            if contour_ref is None:
                contour_ref = contour
                if mapa==0:
                    color_ticks = np.arange(0, vmax + 1, 1)
