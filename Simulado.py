...
for contador in range(ChainSize):
    ...
    if Actualizar==0:
        ...
        Alphaprop[TActualizar]=Alphaprop[TActualizar]+np.random.normal(loc=0,scale=1)#*Cadena[-1][Actualizar][TActualizar])            
        ...
        for i in range(1):
            ...
            Alphaprop[TActualizar]=Actual[TActualizar]+np.random.normal(loc=0,scale=0.1)#*Cadena[-1][Actualizar][TActualizar])
...
for j in range(L):
    ...
    for i in MuestraEfectiva:
        ...
        BurninChain[contador]=Cadena[i][1][0][maps[contador][j]]#beta
...
for j in range(pT):
    ...
    row, col = divmod(j, 4)
    ax = axes[row, col]
    ax.hist(gammahist, alpha=0.4, color=cmap2(j),density=True,bins=8)
...
np.savetxt('LabelClust.csv', LabelClust, delimiter=',')
...
def Mapa(Categoria):
    ClustVI=np.load("ClusterVI.npy") #ClusterVI and ClusterB #best,avg,comp,draws,greedy  
