...
Earthquakes["FaultType"] = Earthquakes["Rake 1"].apply(Tipo)
# Earthquakes=Earthquakes[Earthquakes["FaultType"]!="normal"]
...
Earthquakes=Earthquakes.loc[Earthquakes["Year"]>=minYear]
# Earthquakes=Earthquakes.loc[(Earthquakes["Rake 1"]>45) & (Earthquakes["Rake 1"]<134)]
Earthquakes=Earthquakes.loc[~((Earthquakes["Longitude"]>-98) * (Earthquakes["Latitude"]>18))]
...
CutTime=np.arange(Tmin,Tmax+1,(Tmax-Tmin)/pT)
...
np.savetxt('EarthquakesR.csv', Earthquakes.loc[:,["Longitude","Latitude"]], delimiter=',')
...
for contador in range(ChainSize):
    ...
    if Actualizar==0:
        ...
        Alphaprop[TActualizar]=Alphaprop[TActualizar]+np.random.normal(loc=0,scale=1)            
        # else :
            # Alphaprop[TActualizar]=Alphaprop[TActualizar]+np.random.normal(loc=0,scale=0.0001)            
        ...
        if TActualizar<(pT-1):
            ...
            for ssalt in range(1):
                ...
                if all(Betaprop[TActualizar]>0):
                    rho=min(1,qratio*bp(Cadena[-1][0],Betaprop,Cadena[-1][3],TActualizar )/bp(Cadena[-1][0],ActualCompleto,Cadena[-1][3],TActualizar) )
                else :
                    rho=0
    ...
    if Actualizar==4:
        ...
        results=AsignarZ_Todos(SimPoisson, Cadena[-1], L)
    ...
    if contador % (ChainSize//10)==0: #(ChainSize//100)==0:
        print("Progreso=",contador/ChainSize)
        if contador % (ChainSize//10)==0:
...
Burnin=1000
paralell= get_context("fork").Pool(cpu_count())
logPost = paralell.map(PostParalel, np.arange(len(Cadena[:Burnin])))
paralell.close()     
paralell.join()   
...
logPost = paralell.map(PostParalel, MuestraEfectiva)
paralell.close()     
paralell.join()   
with open('PosteriorMexico.pkl', 'wb') as f:
    pickle.dump(logPost, f)
...
# for i in range(L):
#     CadenaMarginalBetaLabel(i,maps,Cadena,MuestraEfectiva)
#     CadenaMarginalBeta(i)

# Cadena[MuestraEfectiva[0]][1]



# j=L-2
BurninChain=np.ones(len(MuestraEfectiva))
len(BurninChain)
for j in range(L):
    contador=0
    for i in MuestraEfectiva:
        # BurninChain[contador]=Cadena[i][1][0][j]#mal
        BurninChain[contador]=Cadena[i][1][0][maps2[contador][j]]#beta
        # BurninChain[contador]=Cadena[i][4][maps2[contador][j]][0][0]#phi
        contador+=1
    plot_acf(BurninChain,lags=10)
    plt.show()
...
for pTGrupo in range(pT):
    for i in MuestraEfectiva:
        ...
        plt.scatter(np.arange(L),Cadena[i][1][pTGrupo][list(maps2[j].values())], alpha=0.2)   ########Esto reetiqueta
        plt.axhline(1/L)
...
np.savetxt('piMexico.csv', pi, delimiter=',')



len(MuestraEfectiva)
LabelClust=np.zeros((len(MuestraEfectiva),len(Cadena[0][3])))
for i in range(len(MuestraEfectiva)):
    LabelClust[i,:]=Cadena[MuestraEfectiva[i]][3]
    
np.savetxt('LabelClustMexico.csv', LabelClust, delimiter=',')
...
def Mapa(Categoria):
    ClustVI=np.load("ClusterVIEarthquakes.npy") #ClusterVIEarthquakes and ClusterBEarthquakes #best,avg,comp,draws,greedy  
    labelsOpt = ClustVI[Categoria,:].astype(int)-1
    
    cmap3 = plt.cm.get_cmap('plasma', len(np.unique(labelsOpt)))
